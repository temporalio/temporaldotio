...
function VSItem({ header, children, defaultState = false }) {
  ...
  return (
    <li className="border-b border-white">
      <div className={(state ? ' bg-gray5 text-spaceblack' : '') + ' p-4 border-b border-spaceblack text-2xl'}>
        <button onClick={() => setState(!state)} className={"w-full cursor-pointer flex items-center justify-between " + (state ? "hover:text-teal-700" : "hover:text-temporalblue")}>
          <span>{header}</span>
          <span>{state ?
            // x icon
      ...
      {state &&
        <div className={(state ? 'bg-white text-spaceblack' : '') + ' p-4'}>
          {children}
        </div>}
    </li>
  )
